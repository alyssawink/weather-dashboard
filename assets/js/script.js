...
//<---------------------- City Input and Search History
    ...
    function getSearchInput(event) {
     ...
     function searchHistory() {  //Create function to display cities search History stored in localStorage
        ...
        for (let i = 0; i < searchedCitiesArray.length; i++) { //Loop through searched citiies array 
            const displaySearchedCities = searchedCitiesArray[i];
            
            const searchHistoryList = $("<div>").text(displaySearchedCities).addClass("selected"); // Display searched history and store in local storage
...
function getWeather(city) { // Current weather function
    ...
    function updateCurrentWeather(data, city) {
        ...
        let formattedDate = new Date(data.dt * 1000).toLocaleDateString(undefined, dateOptions);
        $(".currentCity").html(`<h3>${city.name} (${formattedDate})</h3>`)
        $(".humidity").text(`Humidity: ${data.main.humidity}%`);
        $(".windSpeed").text(`Wind Speed: ${(data.wind.speed * 2.237).toFixed(2)} mph`);
        $(".temperature").text(`Temperature: ${((data.main.temp - 273.15) * 1.80 + 32).toFixed(2)} F`);
...
//<---------------------- Displays data for 5 Days Weather Forecast
  for (let i = 0; i <=5; i++) {
    ...
    let date = new Date(data.dt * 1000).toLocaleDateString(undefined, options);

    $(`#day${i + 1}Forecast`).text(date); // "i + 1" for day IDs
    $(`#day${i + 1}Icon`).empty().append($(`<img src="http://openweathermap.org/img/wn/${data.weather[0].icon}.png"/>`));
    $(`#day${i + 1}Humidity`).text(`Humidity: ${data.main.humidity}%`);
    $(`#day${i + 1}Temperature`).text(`Temp: ${((data.main.temp - 273.15) * 1.80 + 32).toFixed(2)} F`);
    $(`#day${i + 1}Wind`).text(`Wind: ${(data.wind.speed * 2.237).toFixed(2)} mph`);
  }
}
 ...
 function getUVindex(lat, long) {
        ...
        $.ajax({
            ...
            .done(function(responseUVI) {
                ...
                let color;
                if (uvIndex <= 2.99) { color = "olivedrab"; }
                else if (uvIndex <= 5.99) { color = "gold"; }
                else if (uvIndex <= 7.99) { color = "darkorange"; }
                else { color = "firebrick"; }
                $("#uvIndex").css({"background-color": color, "display": "block", "border-radius": "12px", "padding": "1.5%", "max-width": "20%"});
            })
            .fail(function(jqxhr, textStatus, error) {
                console.log("Request Failed: " + textStatus + ", " + error);
            });
